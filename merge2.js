...
async function handleWebhookRequest(reqBody) {
    ...
    try {
      ...
      for (const branch of branchRows) {
        ...
        if (!isBranchIncluded) {
          logger.info(`分支 "${svn_branch_name}" 不在请求的 paths 中，跳过检查`);
          continue; // 如果当前分支不在请求的 paths 中，跳过
        }
  
        hasMatchingBranch = true; // 标记有匹配的分支
        logger.info(`正在检查分支 "${svn_branch_name}" 的锁定状态`);
        ...
        if (svn_lock_whitelist.split(',').filter(Boolean).includes(user_name)) {
          ...
          logger.info(`用户 "${user_name}" 在永久白名单中，允许提交分支 "${svn_branch_name}"`);
          continue;
        }
        ...
        if (index !== -1) {
          ...
          logger.info(`用户 "${user_name}" 在一次性白名单中，已移除并允许提交分支 "${svn_branch_name}"`);
          continue;
        }
  
        // 如果以上条件都不满足，则拒绝提交
        responseMessages.push(`分支 "${svn_branch_name}" 锁定状态为 1，且用户 "${user_name}" 不在白名单中，拒绝提交`);
        logger.error(`分支 "${svn_branch_name}" 锁定状态为 1，且用户 "${user_name}" 不在白名单中，拒绝提交`);
        return { status: 500, message: `提交被拒绝：分支 "${svn_branch_name}" 已锁定，且用户 "${user_name}" 不在白名单中。` };
      }
