...
function addDisposableWhitelist(branchName, whitelistUser) {
  return new Promise((resolve, reject) => {
    // 第一步：检查分支是否存在
    const checkQuery = 'SELECT * FROM tb_branch_info WHERE svn_branch_name = ?';
    pool.query(checkQuery, [branchName], async (checkError, checkResults) => {
      ...
      pool.query(updateQuery, [whitelistUser, branchName], (updateError, updateResults) => {
        if (updateError) {
  ...
  try {
    ...
    if (lockMatch) {
      ...
      if (success) {
        ...
        replyMessage = `为分支 ${branchName} 增加一次性白名单用户 ${whitelistUser} 失败，请检查分支是否存在`;
      }
