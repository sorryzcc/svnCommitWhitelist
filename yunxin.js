...
app.post('/', async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(`Permanent_whitelist Query Result: ${JSON.stringify(permanentRows)}`);
      ...
      if (permanentRows.length > 0) { // 如果找到匹配项
        ...
        const [rows] = await conn.execute(
          ...
          [request_data.user_name, request_data.branch || 'trunk']  // 默认分支为 trunk
        );
        console.log(`One-time Whitelist Query Result: ${JSON.stringify(rows)}`);
    ...
    console.error("Detailed Error:", error.stack); // 打印完整错误信息
...
app.post('/add-one-time-whitelist', async (req, res) => {
  try {
    ...
    // 验证必填字段
    if (!user_name || !branch) {
      throw new Error('user_name 和 branch 是必填参数');
    }
    ...
    try {
      ...
      console.log(`Existing Record Query Result: ${JSON.stringify(existing)}`);
      ...
      if (existing.length > 0) {
        ...
        console.log(`Updated existing record with ID: ${existing[0].id}`);
      } else {
        // 如果不存在，则插入新记录
        await conn.execute(
        ...
        console.log(`Inserted new record for user: ${user_name}, branch: ${branch}`);
      }
