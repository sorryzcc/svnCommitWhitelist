...
function addDisposableWhitelist(branchName, whitelistUser) {
  return new Promise((resolve, reject) => {
    const query =
      'INSERT INTO tb_branch_info (svn_branch_name, svn_lock_disposable_whitelist) VALUES (?, ?) ON DUPLICATE KEY UPDATE svn_lock_disposable_whitelist = ?';
    pool.query(query, [branchName, whitelistUser, whitelistUser], (error, results) => {
      if (error) {
